...
terraform apply
```
From the output, take your four name servers and upload them to your domain provider and wait for DNS propagation (can take up to 72 hours). The Apache web server should then be available at `https://<domain>.<extension>` or `https://www.<domain>.<extension>`. Alternatively, you may HTTPS to the hostname of the ALB (provided by the Terraform output), however the site will not be secure as the certificate is a mismatch.
